...
class ConnectionViewController: UIViewController, UITextViewDelegate {
    ...
    @IBOutlet var mqttStatus: UITextView!
    ...
    @objc var profileViewController: UIViewController!
    @objc var messagingViewController: UIViewController!
    ...
    func mqttEventCallback( _ status: AWSIoTMQTTStatus ) {
        DispatchQueue.main.async {
            ...
            switch status {
            case .connecting:
                self.mqttStatus.text.append( "Connecting...")
                print( tabBarViewController.mqttStatus )
                self.mqttStatus.text = tabBarViewController.mqttStatus
            ...
            case .connected:
                ...
                self.mqttStatus.text = "Using certificate:\n\(certificateId!)\n\n\nClient ID:\n\(uuid)"
                
                tabBarViewController.viewControllers = [ self, self.publishViewController, self.subscribeViewController, self.messagingViewController, self.profileViewController ]
                
            case .disconnected:
                tabBarViewController.mqttStatus = "Disconnected"
                print( tabBarViewController.mqttStatus )
                self.activityIndicatorView.stopAnimating()
                self.mqttStatus.text = nil
    ...
    func handleConnectViaCert() {
        ...
        if (certificateId == nil) {
            DispatchQueue.main.async {
                self.mqttStatus.text = "No identity available, searching bundle..."
            }
            let certificateIdInBundle = searchForExistingCertificateIdInBundle()
            
            if (certificateIdInBundle == nil) {
                DispatchQueue.main.async {
                    self.mqttStatus.text = "No identity found in bundle, creating one..."
                }
                createCertificateIdAndStoreinNSUserDefaults(onSuccess: {generatedCertificateId in
                    let uuid = UUID().uuidString
                    self.mqttStatus.text = "Using certificate: \(generatedCertificateId)"
    ...
    func searchForExistingCertificateIdInBundle() -> String? {
        ...
        DispatchQueue.main.async {
            self.mqttStatus.text = "found identity \(certId), importing..."
        }
        if AWSIoTManager.importIdentity( fromPKCS12Data: data, passPhrase:"", certificateId:certId) {
            ...
            DispatchQueue.main.async {
                self.mqttStatus.text = "Using certificate: \(certId))"
        ...
        self.iotManager.createKeysAndCertificate(fromCsr: csrDictionary) { (response) -> Void in
            guard let response = response else {
                DispatchQueue.main.async {
                    self.connectButton.isEnabled = true
                    self.activityIndicatorView.stopAnimating()
                    self.mqttStatus.text = "Unable to create keys and/or certificate, check values in Constants.swift"
                }
    ...
    func handleDisconnect() {
        ...
        activityIndicatorView.startAnimating()
        mqttStatus.text = "Disconnecting..."
        
        DispatchQueue.global(qos: DispatchQoS.QoSClass.default).async {
            self.iotDataManager.disconnect();
            DispatchQueue.main.async {
                self.connected = false
                tabBarViewController.viewControllers = [ self, self.profileViewController ]
            }
        }
    }
    ...
    */
    override func viewDidLoad() {
        ...
        profileViewController = tabBarViewController.viewControllers![4]
        messagingViewController =  tabBarViewController.viewControllers![5]
        
        tabBarViewController.viewControllers = [ self, profileViewController ]
        mqttStatus.resignFirstResponder()
    ...
    func setupViewComponents() {
        ...
        y += spacer + buttonHeight
        mqttStatus = UITextView(frame: CGRect(x: 5, y: y, width: 300, height: labelHeight))
        ...
        self.view.addSubview(mqttStatus)
